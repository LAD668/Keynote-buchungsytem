-- Fix: storage.objects RLS enabling can fail with "must be owner of table objects".
-- In most Supabase projects, RLS on storage.objects is already enabled, so we only (re)create policies.
